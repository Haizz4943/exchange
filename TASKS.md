...
- [x] **TASK-042 — Matching: seek-to-end `market-data.events.v1` lúc startup + retention ngắn cho topic** - (why: bug — topic firehose không retention phình ~13,5M dòng; matching restart/lag resume từ committed offset → replay hàng triệu external trade CŨ ở giá lỗi thời → khớp lệnh sai giá, đo thực tế ~98 phút mới đuổi kịp live. Fix 2 lớp: (1) `MarketDataEventsConsumer extends AbstractConsumerSeekAware` seek-to-end khi assign partition (bỏ backlog, khớp giá live ngay), KHÔNG đụng ordering của orders/matching topics; (2) market-data `KafkaAdmin(modifyTopicConfigs=true)` + `NewTopic` đặt `retention.ms=600000`/`segment.ms=60000` chặn topic phình. Deviation/quyết định ghi [matching DECISIONS](services/matching/DECISIONS.md) + [marketdata DECISIONS](services/marketdata/DECISIONS.md). Liên quan TASK-041 (cùng gốc firehose, khác lớp)). E2E verified 2026-06-26: reset committed offset → LAG ~10,9M → restart matching → seek-to-end → LAG 0 ngay + track live (~500 lag từ feed); orders offset không đổi; topic báo `retention.ms=600000` đã áp. 53/53 matching tests pass; merged main (`04e51d6`)
