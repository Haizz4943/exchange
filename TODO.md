...
3. 🟡 **Matching Engine** — fill theo Market Data → `TradeExecuted` → Wallet/Order cập nhật. Core matching + unit test xong; còn integration test thật. Gateway đã chừa mapping WS `matching.events.v1` → channel `orders`.
